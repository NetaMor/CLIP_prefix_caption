...
class Predictor():
    ...
    def predict(self, image_path, model, use_beam_search):
        """Run a single prediction on the model"""
        model = self.models[model]
        image = io.imread(image_path)
        #if image.shape[0]>512:
        #    image = transform_img(image_path)
...
def predict_rgb(path_weights_model, path_images):
    prefix_length = 10
    ...
    for doc in os.listdir(path_images):
        for file in os.listdir(path_images+"/"+doc):
            UPLOADED_FILE = os.path.join(path_images,doc, file)
            print(file)
...
def predict_rgb_and_ir(path_weights_model, path_images):
    prefix_length = 10
    model = 'coco'  # 'conceptual-captions'
    Pre = Predictor(path_weights_model, model)
    gen_to_save = pd.DataFrame(columns=['img_type','img_name', 'caption_gen2', 'caption_beamGen'])
    for file in os.listdir(path_images):
        UPLOADED_FILE = os.path.join(path_images, file)
        print(file)
    ...
    gen_to_save.to_csv('/home/dvir/Desktop/Projects/CLIP_prefix_caption/data/koronos/models_results/results_ir_rgb/'+path_weights_model.split('/')[-1].split('.')[0]+'.csv')
...
if __name__ == '__main__':
    ...
    #predict_rgb(path_weights_model, path_images)
